...
import { Folder, FileCode, Server, Database, User, Shield, Terminal, Moon, Sun, Home } from 'lucide-react';
          ...
          {navItems.map((item) => (
            <li key={item.id}>
              <button 
                ...
                className={`
                  flex items-center gap-2 w-full px-2 py-2 text-sm text-left rounded group transition-all
                  ${activeView === item.id 
                    ? (isDark ? 'bg-white/10 text-white font-bold' : 'bg-black/10 text-black font-bold') 
                    : 'opacity-70 hover:opacity-100 hover:bg-white/5'}
                `}
              >
                <item.icon size={15} className={`shrink-0 ${activeView === item.id ? (isBash ? 'text-green-400' : 'text-blue-400') : (isBash ? 'text-blue-400' : 'text-yellow-400')} group-hover:scale-110 transition-transform`} />
                <span className="truncate">{item.label}</span>
                <span className={`opacity-0 group-hover:opacity-30 text-xs ml-auto shrink-0 ${activeView === item.id ? 'opacity-30' : ''}`}>
                  {isBash ? '.sh' : '.ps1'}
                </span>
              </button>
            </li>
