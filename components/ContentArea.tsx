...
const SectionHeader: React.FC<{ 
  ...
  isVisible: boolean;
  onTypeComplete: () => void;
}> = ({ bashCmd, psCmd, isVisible, onTypeComplete }) => {
  ...
  return (
    <div className="mb-6 font-bold text-sm md:text-base break-all flex flex-wrap">
...
const AnimatedBlock: React.FC<{
  ...
  delay?: number;
}> = ({ children, startAnimation, delay = 0 }) => {
  ...
  return (
    <div className={`transition-all duration-700 transform ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      {children}
    </div>
  );
};
...
export const ContentArea: React.FC = () => {
  ...
  // Render Logic for Section Header + Content
  const renderSection = (
    isVisible: boolean, 
    bashCmd: string, 
    psCmd: string, 
    ...
    // We use local state to trigger the content *after* the command finishes typing
    // eslint-disable-next-line react-hooks/rules-of-hooks
    const [headerFinished, setHeaderFinished] = useState(false);

    // If section becomes invisible (user scrolls up), reset? 
    // Usually better to keep it visible once shown.
    ...
    return (
      <>
        <SectionHeader 
          ...
          psCmd={psCmd} 
          isVisible={isVisible} 
          onTypeComplete={() => setHeaderFinished(true)} 
        />
        <AnimatedBlock startAnimation={headerFinished} delay={100}>
          {content}
        </AnimatedBlock>
      </>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-24">
      ...
      {/* HEADER SECTION */}
      {/* This one is visible immediately, so we don't need intersection observer as much, 
          but we use it for consistency or just render directly. 
          Let's make the H1 type out too. */}
      <section className="min-h-[50vh] flex flex-col justify-center">
         <div className="mb-4">
            <Typewriter text="Initializing User Session..." speed={30} showCursor={false} className="text-xs uppercase tracking-widest opacity-60" />
         </div>
         <h1 className="text-4xl md:text-6xl font-bold mb-4">
            <Typewriter text="Bhashkar " speed={80} showCursor={false} startDelay={1000} />
            <span className={`${accentColor}`}>
               <Typewriter text="Gupta" speed={80} showCursor={false} startDelay={1800} />
            </span>
         </h1>
         <div className="text-xl md:text-2xl opacity-90 mb-6 min-h-[2rem]">
            <AnimatedBlock startAnimation={true} delay={2500}>
              System Administrator | Blue Team Aspirant | OS Developer
            </AnimatedBlock>
         </div>
         <div className="text-lg opacity-70 border-l-4 border-current pl-4 italic min-h-[2rem]">
            <AnimatedBlock startAnimation={true} delay={3000}>
      ...
      <SectionWrapper id="experience">
        {(isVisible) => renderSection(
          isVisible,
          "./history | grep 'Work'",
          "Get-History | Select-String 'Work'",
      ...
      <SectionWrapper id="skills">
        {(isVisible) => renderSection(
          isVisible,
          "printenv | grep SKILLS",
          "Get-Variable -Name Skills",
      ...
      <SectionWrapper id="contact" className="mb-12">
        {(isVisible) => renderSection(
          isVisible,
          "./contact.sh --message",
          "Send-MailMessage -To 'Me'",
